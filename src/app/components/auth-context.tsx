...
type User = {
  ...
  username: string;
  emoji: string;
  bio?: string; // Custom bio for profile
};
...
type AuthContextType = {
  ...
  updateBio: (bio: string) => Promise<void>; // New function for bio
};
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const updateBio = async (bio: string) => {
    ...
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-4053ac61/profile/${user.id}/bio`,
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
  ...
  return (
    <AuthContext.Provider value={{ user, loading, signIn, signUp, signOut, updateEmoji, updateBio }}>
      {children}
